...
#else 
#define STM_SECTOR_SIZE	2048//大容量产品
#endif		 
u16 STMFLASH_BUF[STM_SECTOR_SIZE/2];//最多是1K字即2K字节
...
void STMFLASH_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
	...
	{	
		...
		{
			if(STMFLASH_BUF[secoff+i]!=0XFFFF)break;//需要擦除  若不擦除，则0位不能被写1，得不到正确存储  	  
		}
		if(i<secremain)//需要擦除
		{
